...
void codigoProcesoHijo	( int [], int [], int, int [] );
void codigoProcesoPadre	( int [] );
...
int main()
{
	...
	for( np = 0; np < NUM_PROC; np++ )
	{
		...
		{
			codigoProcesoHijo( seno, rxx, np, pipefd );
		}
	}
...
void codigoProcesoHijo( int x[], int rxx[], int id, int pipefd[] )
{
	register int n, l;
	int fun_creciente = 0;
	int muestra;
...
{
	register int i;
	int pid, salida, muestra, muestrafinal = 0;

	close( pipefd[1] );
	for( i = 0; i < NUM_PROC; i++ )
	{
		read( pipefd[0], &muestra, 4 );
		pid = wait( &salida );		
